# Running the APM Book Fleet Autonomously

This folder holds launchers that run the book **agent fleet** unattended in the backend.
...
| **Driver** | `.github/prompts/run-book.prompt.md` | The one prompt you launch; it orchestrates everything |

You only ever launch the **driver**. It reads the `book-orchestration` skill and dispatches the
...
`--allow-all-tools` lets it install packages, run commands, and commit without prompts.

```powershell
# from the repo root
copilot -p "$(Get-Content -Raw .github/prompts/run-book.prompt.md)" --allow-all-tools
```

```bash
# bash equivalent
copilot -p "$(cat .github/prompts/run-book.prompt.md)" --allow-all-tools
```
...
reference research for a chapter, or several chapters in the same wave. Interactively you can also
...
- Host tokens for private sources go in a gitignored `.env`; examples use public sample packages,
  so a full run needs no secrets unless you point chapters at private repos.
